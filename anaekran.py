...
from guncel import GuncelPenceresi
...
class Anaekran(QMainWindow,Ui_AnaEkran):
    ...
    def kitap_listele(self):
        ...
        for a in kitaplar:
            ...
            self.tableWidget.setItem(k, 4, QTableWidgetItem(str(a[4])))
            k += 1

    def kitap_silme(self):
        print("kitap silme tıklandı")
        silinecek = input("silmek istediğiniz kitabın id'sini giriniz")
        baglanti.guncelle(silinecek)

    # def kitap_guncelle(self):
    #     self.setCentralWidget(GuncelPenceresi(self.k_id))
